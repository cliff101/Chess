...
int* RandAIPlayer::SelectChess(Board& const board, int playerid, GameManager::movetype prevplayermovetype)
{
	srand(time(NULL));
	while (true) {
...
int RandAIPlayer::SelectMoveOption(std::vector<GameManager::movetype>& avail, int selectedchesstype)
{
	srand(time(NULL));
...
int RandAIPlayer::SelectPromote(Board& const board, int Pos[2])
{
	srand(time(NULL));
	int selected = rand() % 3;
	return selected + 2;
...
void RandAIPlayer::OnMove(Board& const board, int* frompos, GameManager::movetype to)
{
...
void RandAIPlayer::OnPromote(Board& const board, int Pos[2], Board::basechess outchess)
{
